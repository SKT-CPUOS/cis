...
static int add_block_to_pool(void)
{
  ...
  if (!block->data) {
      printk("allocate dma cache failed\n");
      kfree(block);
      return -1;
  }
...
static int scanner_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    //printk(KERN_INFO "Memory block pointer set to vma: phys=0x%08x, virt=0x%p\n",
       // (u32)(block->paddr), block->data);
    printk("scanner_mmaps count=%u\n", count);
    // 将内存块映射到用户空间
    if (remap_pfn_range(vma, vma->vm_start,
                        block->paddr >> PAGE_SHIFT,
                        scanner.cache_size, vma->vm_page_prot)) {
        printk(KERN_ERR "Failed to remap memory block\n");
        return -EAGAIN;
    }
   // printk(KERN_INFO "Memory block successfully mapped to userspace: size=%zu\n",
       // scanner.cache_size);
    return 0;
}
...
static long scanner_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case CMD_ALLOC_MEM: {
            ...
            printk("Memory pool add sucesss\r\n");
            block = list_first_entry(&memory_pool, struct memory_block, list);
            printk("list first pool sucesss\r\n");
            writel((u32)(block->paddr), &regs->cache_addr);
            printk("write acche_adrr sucesss\r\n");
            break;
        }
        case CMD_SET_DPI : {
            if (arg > 2) 
